...
describe('Login', () => {
  ...
  beforeEach(() => {
    AsyncStorage.getItem = jest.fn(() =>
      Promise.resolve(JSON.stringify(getItemResponse)),
    );
  ...
  it('Should have an input to set the user email', done => {
    ...
    setTimeout(() => {
      expect(
        instance.findByProps({className: 'email-input'}).props.autoCompleteType,
      ).toBe('email');
      expect(
        instance.findByProps({className: 'email-input'}).props.keyboardType,
      ).toBe('email-address');
      done();
    });
  });
  ...
  it('Should have an input to set the user password', done => {
    ...
    setTimeout(() => {
      expect(
        instance.findByProps({className: 'password-input'}).props
          .autoCompleteType,
      ).toBe('password');
      expect(
        instance.findByProps({className: 'password-input'}).props
  ...
  it('Should have a button to submit the form', done => {
    ...
    setTimeout(() => {
      expect(
        instance.findByProps({className: 'submit-login'}).props.disabled,
      ).toBe(true);
      expect(
        instance.findByProps({className: 'submit-login'}).props.title,
      ).toBe('Entrar');
      done();
    });
  });
  ...
  it('Should enable the submit button when email and password are valid', done => {
    ...
    instance
      .findByProps({className: 'email-input'})
      .props.onChangeText('rafael@gmail.com');
    instance
      .findByProps({className: 'password-input'})
  ...
  it('Should disable the submit button when the email are invalid', done => {
    ...
    instance
      .findByProps({className: 'email-input'})
      .props.onChangeText('rafael@gmail.c');
    instance
      .findByProps({className: 'password-input'})
      .props.onChangeText('123');
  ...
  it('Should make a request when click on submit-login with the valid data', done => {
    ...
    setTimeout(() => {
      ...
      expect(postSpy).toHaveBeenCalledWith(
        'https://api.codenation.dev/v1/user/auth',
        {
...
describe('Acceleration', () => {
  ...
  it('Should get the accelerations', () => {
    ...
    expect(asyncSpy).toHaveBeenCalledWith(
      'https://api.codenation.dev/v1/acceleration',
    );
  });
  ...
  it('Should have a loading waiting for the accelerations to be loaded', done => {
    ...
    setTimeout(() => {
      expect(instance.findByType(AccelerationItem).props.item).toEqual(
        accelerations[0],
      );
      done();
    });
...
describe('Profile', () => {
  ...
  it('Should get the profile', done => {
    ...
    setTimeout(() => {
      expect(asyncSpy).toHaveBeenCalledWith(
        'https://api.codenation.dev/v1/me/profile',
        {
          headers: {
            Authorization: '123',
          },
